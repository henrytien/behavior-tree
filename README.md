...
专用的编辑器分支版本：`behavior3editor`   
编译好的桌面版：`behavior3editor/releases`

## 示例 Examples

- [load_from_tree](https://github.com/henrytien/behavior-tree/tree/master/examples/load_from_tree)  ：从导出的树文件加载示例
- [load_from_project](https://github.com/henrytien/behavior-tree/tree/master/examples/load_from_project) ：从导出的工程文件加载示例
- [load_from_rawproject](https://github.com/henrytien/behavior-tree/tree/master/examples/load_from_rawproject) ：从原生工程文件加载示例
- [subtree](https://github.com/henrytien/behavior-tree/tree/master/examples/subtree) ：子树的使用示例（需要专用的编辑器分支版本 `behavior3editor`）
- [mmoarpg](examples/mmoarpg/zt.b3)：一个mmoarpg类型游戏的行为示例，可使用桌面版编辑器打开可以查看(Projects->open project)

## 完整示例

io类游戏示例：`h5game/server`  
...
```
- Q:如何设计打断一个进行中的状态？   
A:参考https://github.com/henrytien/behavior-tree/issues/15
## TODO
- [ ] 参数类型化
- [ ] 参数支持传递黑板值利用格式“@变量名”
- [ ] 子树支持自定义参数传递
